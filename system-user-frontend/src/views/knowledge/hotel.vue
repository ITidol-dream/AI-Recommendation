  ...
  <div class="hotel">
    ...
    <div class="attractions1">
      <div class="attractions2">
        <el-select size="small" style="margin-left:20px" v-model="search.attractions" placeholder="请选择课程名称">
            <el-option v-for="(item,index) in attractions" :key="index"
            :label="item.name"
            :value="item.name">
            </el-option>
        </el-select>
        <el-input size="small" style="width:300px;margin-left:20px" v-model="search.name" placeholder="请输入相关内容"></el-input>
      ...
      <div class="attractions3">
        <div class="index5" v-for="(item,index) in tableData" :key="index" style="margin-top:15px">
          ...
          <div class="index6">
            ...
            <div class="index9" style="margin-bottom:10px;cursor: pointer;margin-top:10px" @click="toInfo(item.id)">
              学习
            </div>
          </div>
        </div>
